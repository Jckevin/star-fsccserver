	...
	public void printTerMap() {
		...
		while (iter.hasNext()) {
			...
			if (entry.getKey().equals("800") || entry.getKey().equals("801") || entry.getKey().equals("810")) {
				...
				logger.error("value status : {},{},{},{},{},{}", entry.getValue().getStatus(),
						entry.getValue().getCallDirection(), entry.getValue().getCallType(),
						entry.getValue().getCallUUid(), entry.getValue().getPeerNumber(),
						entry.getValue().getPeerUUid());
			}
