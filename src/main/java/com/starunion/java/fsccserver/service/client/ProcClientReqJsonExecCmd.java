...
/**
 * @author Lings
 * @date Mar 26, 2016 5:15:31 PM
 * 
 */
@Service
public class ProcClientReqJsonExecCmd {
	private static final Logger logger = LoggerFactory.getLogger(ProcClientReqJsonExecCmd.class);

	@Autowired
	CallableFsExecCmdProc taskInt;
	@Autowired
	JsonMsgService jsonService;
	private RspMsgBase msgRsp = new RspMsgBase();

	public ProcClientReqJsonExecCmd() {

	}

	public String execCmdRecord(ReqMsgSysGenOper msg) {
		String uuid = ServerDataMap.terStatusMap.get(msg.getCallee()).getCallUUid();
		Date time = new Date();
