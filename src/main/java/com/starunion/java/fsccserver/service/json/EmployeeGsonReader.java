...
public class EmployeeGsonReader {
	...
	public static void main(String[] args) throws IOException {
		InputStream is = new FileInputStream("C:\\employee.txt");
	...
	private static String parseJSON(JsonReader reader, Employee emp, List<Long> phoneNums, String key,
		...
		while (reader.hasNext()) {
			...
			case NAME:
				...
				if ("properties".equals(key))
					insidePropertiesObj = true;
				break;
			case BOOLEAN:
				if ("permanent".equals(key))
					emp.setPermanent(reader.nextBoolean());
				else {
	...
	private static void setStringValues(Employee emp, String key, String value, boolean insidePropertiesObj) {
		if ("name".equals(key))
			emp.setName(value);
		else if ("role".equals(key))
			emp.setRole(value);
		else if ("cities".equals(key))
			emp.getCities().add(value);
		else if ("street".equals(key))
			emp.getAddress().setStreet(value);
		else if ("city".equals(key))
			emp.getAddress().setCity(value);
		else {
