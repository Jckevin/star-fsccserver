...
public class EmployeeGsonWriter {
	...
	public static void main(String[] args) throws IOException {
		...
		// writing on console, we can initialize with FileOutputStream to write
		// to file
		...
		writer.name("street").value(emp.getAddress().getStreet()); // "street":
																	// "BTM 1st
																	// Stage"
		writer.name("city").value(emp.getAddress().getCity()); // "city":
																// "Bangalore"
		writer.name("zipcode").value(emp.getAddress().getZipcode()); // "zipcode":
																		// 560100
		writer.endObject(); // }
		writer.name("phoneNumbers").beginArray(); // "phoneNumbers": [
		for (long num : emp.getPhoneNumbers())
			writer.value(num); // 123456,987654
		writer.endArray(); // ]
		writer.name("role").value(emp.getRole()); // "role": "Manager"
		writer.name("cities").beginArray(); // "cities": [
		for (String c : emp.getCities())
			writer.value(c); // "Los Angeles","New York"
		writer.endArray(); // ]
		writer.name("properties").beginObject(); // "properties": {
		Set<String> keySet = emp.getProperties().keySet();
		for (String key : keySet)
			writer.name("key").value(emp.getProperties().get(key));// "age": "28
																	// years","salary":
																	// "1000 Rs"
		writer.endObject(); // }
		writer.endObject(); // }
