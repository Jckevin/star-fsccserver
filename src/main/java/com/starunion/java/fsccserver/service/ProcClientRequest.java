...
//		daoUserSip.batchInsertUser(data[1], data[2], data[3], data[4]);
	...
	private StringBuffer procExtenGpsTrailShowByTime(String[] data) {
		...
		for (UserGpsTrail trail : gpsList) {
			...
			int len = time.length();
			buff.append(time.substring(0, len-2)).append(":");
			buff.append(trail.getType());
			buff.append("\r\n");
		}
		return buff;
	}
	...
	private int procExtenGpsAdd(String[] data) {
		...
		UserGpsInfo extenInfo = daoUserGpsInfo.findByNumber(exten);
		if (extenInfo != null) {
			daoUserGpsInfo.updateGpsByNumber(exten, lng, lat, type);
			daoUserGpsTrail.addExtenGpsTrail(exten, lng, lat, type);
			return -1;
		}
		int res = daoUserGpsInfo.addExtenGps(exten, lng, lat, type);
		return res;
	}
